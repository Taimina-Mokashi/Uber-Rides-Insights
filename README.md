🚖 Uber Ride Analytics Dashboard – Power BI
📌 Project Overview

This Power BI dashboard provides an interactive analysis of Uber ride bookings, revenue, trip performance, and customer/driver insights. It helps stakeholders understand booking trends, ride completion status, vehicle performance, and regional demand patterns for better operational and business decisions.

🎯 Objectives

Analyze overall ride performance and booking trends
Track revenue, total bookings, and trip distance metrics
Understand booking outcomes (completed, cancelled, incomplete)
Identify top pickup and drop locations
Evaluate driver and customer satisfaction ratings



📊 Key Metrics

Total Revenue: 9M
Total Bookings: 27K
Highest Trip Distance: 50 km
Completed Rides: 17K
Cancelled Rides: 7K
Incomplete Rides: 4K



📈 Dashboard Features

1️⃣ Booking Trends
Monthly and quarterly booking analysis
Visualizes fluctuations in ride demand across the year

2️⃣ Booking Status Overview
Donut chart showing:
Completed rides
Cancelled by driver
Cancelled by customer
No driver found
Incomplete rides

3️⃣ Vehicle Type Analysis
Pickup distribution by vehicle type
Focused on 4-Seated Cab performance

4️⃣ Location Insights
Top Pickup Location: Ashram (117 rides)
Top Drop Location: Narsinghpur (126 rides)

5️⃣ Ratings & Experience

Driver Rating: 4.23 ⭐
Customer Rating: 4.41 ⭐



🛠 Tools & Technologies

Power BI Desktop
DAX for calculated measures
Data Modeling & Visualization
Interactive slicers (Month / Quarter)



📂 Dataset

Simulated Uber ride booking data
Includes booking status, vehicle type, trip distance, revenue, location, and ratings
(Dataset used for learning and portfolio purposes)



📌 Insights Gained

Majority of rides are successfully completed (~61%)
Cancellations form a significant portion, indicating optimization opportunities
Demand peaks during mid-year months
High customer and driver ratings indicate strong service quality



🚀 How to Use

Download the .pbix file
Open in Power BI Desktop
Use slicers to explore monthly and quarterly trends
Hover over visuals for detailed insights



📬 Contact

If you’d like to discuss this dashboard or similar analytics projects, feel free to connect with me on LinkedIn or reach out directly.
